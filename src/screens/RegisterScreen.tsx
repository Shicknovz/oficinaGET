...
interface Props {
  onRegister: (nome: string, email: string, senha: string) => void | Promise<void>;
  onBack?: () => void;
}
...
export default function RegisterScreen({ onRegister, onBack }: Props) {
  ...
  const [errors, setErrors] = useState<{ name?: string; email?: string; senha?: string; form?: string }>({});
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      await onRegister(name.trim(), email.trim(), senha);
    } catch (error) {
      setErrors({ form: error instanceof Error ? error.message : 'Não foi possível criar a conta.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const formContent = (
    <>
      ...
      {errors.form ? <Text style={[styles.formError, { color: t.danger }]}>{errors.form}</Text> : null}
    </>
  );
...
const styles = StyleSheet.create({
  form: { marginTop: 4 },
  formError: { fontSize: 13, marginTop: 10, textAlign: 'center' },
  tipCard: {
    borderWidth: 1,
    borderRadius: 16,
    padding: 14,
